...
internals.Composer.prototype.compose = function (callback) {
    ...
    this.settings.forEach(function (set) {
        ...
        Object.keys(set.servers).forEach(function (serverName) {
            ...
            // Special case port values so we can pass environment vars for heroku compatibilty
            if (port != parseInt(port)) {
                port = parseInt(process.env[port]);
            }

            pack.server(serverName, new Server(serverOptions.host, port, serverOptions.config), { labels: serverOptions.labels });
        });
